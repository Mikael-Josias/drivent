  ...
  describe('when token is valid', () => {
    ...
    describe('when body is valid', () => {
      ...
      it('should respond with status 403 when booking id is invalid', async () => {
        ...
        const response = await server.put('/booking/invalid').set('Authorization', `Bearer ${token}`).send(validBody);
        expect(response.status).toBe(httpStatus.FORBIDDEN);
      });
      it('should respond with status 404 when room does not exist', async () => {
        const user = await createUser();
        ...
        const response = await server.put('/booking/').set('Authorization', `Bearer ${token}`).send(validBody);
        expect(response.status).toBe(httpStatus.NOT_FOUND);
      });
      it('should respond with status 403 when rooms capacity is full', async () => {
        const user = await createUser();
      ...
      it('should respond with status 200 and booking id when altered', async () => {
        const user = await createUser();
        const token = await generateValidToken(user);
